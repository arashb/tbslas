...
void get_vorticity_field_wrapper(const real_t* points_pos, int num_points,
                                 real_t* points_values) {
  real_t omega = 1;
  real_t freq = 0;
  tbslas::get_vorticity_field_tv<real_t, 3>(points_pos, num_points, tcurr,
                                            points_values, omega, freq);
}
template <typename real_t, int sdim>
void get_gaussian_field_atT(const real_t* points_pos, int num_points,
                            real_t* out) {
  real_t omega = 1.0;
  ...
  tbslas::get_gaussian_field_3d<real_t, sdim>(
      points_pos, num_points, out, xc, yc, zc, A, sigma_x, sigma_y, sigma_z);
}

template <class real_t>
void get_vorticity_field_tv_wrapper(const real_t* points_pos, int num_points,
                                    real_t* points_values) {
  real_t omega = 1;
  real_t freq = 2 * PI;
  tbslas::get_vorticity_field_tv<real_t, 3>(points_pos, num_points, tcurr,
...
template <class real_t>
void get_taylor_green_field_tv_wrapper(const real_t* coord, int n,
                                       real_t* out) {
  real_t freq = 2 * PI;
  tbslas::get_taylor_green_field_tv(coord, n, tcurr, out, freq);
}

template <typename real_t>
void get_gaussian_field_tv_wrapper(const real_t* points_pos,
                                   const int num_points, real_t* out) {
  real_t omega = 1;
  real_t freq = 2 * PI;
  ...
  /* real_t theta  = theta0 + omega*(tcurr - tcurr_init - (cos(freq*tcurr) -
   * cos(freq*tcurr_init))/freq); */
  ...
  tbslas::get_gaussian_field_3d<real_t, 3>(points_pos, num_points, out, xc, yc,
                                           zc, A, sigma_x, sigma_y, sigma_z);
}

template <class real_t>
void get_diffusion_kernel_tv_wrapper(const real_t* coord, int n, real_t* out) {
  ...
  real_t r = sqrt((xci - 0.5) * (xci - 0.5) + (yci - 0.5) * (yci - 0.5));
  real_t theta =
      theta0 + omega * (tcurr - tcurr_init -
                        (cos(freq * tcurr) - cos(freq * tcurr_init)) / freq);

  real_t xc = 0.5 + r * cos(theta);
  real_t yc = 0.5 + r * sin(theta);
  real_t zc = zci;

  tbslas::diffusion_kernel(coord, n, out, TBSLAS_DIFF_COEFF, tcurr, amp, xc, yc,
                           zc);
}
...
template <typename real_t, int sdim>
void get_slotted_cylinder_atT(const real_t* points_pos, int num_points,
                              real_t* out) {
  real_t xc = 0.5;
  real_t yc = 0.5;
  real_t zc = 0.5;
  ...
  real_t w = 0.1;
  real_t a = tcurr;
  tbslas::get_slotted_cylinder<real_t, sdim>(points_pos, num_points, out, xc,
                                             yc, zc, R, w, a);
}

template <class real_t>
void get_guassian_kernel_wraper(const real_t* coord, int n, real_t* out) {
  const real_t xc = 0.7;
  const real_t yc = 0.7;
  const real_t zc = 0.7;
...
template <class real_t>
void get_multiple_guassian_kernel_wraper(const real_t* coord, int n,
                                         real_t* out) {
  // FIRST GAUSSIAN
...
template <class real_t>
void get_taylor_green_field_wrapper(const real_t* coord, int n, real_t* out) {
  tbslas::get_taylor_green_field(coord, n, out);
}

template <class real_t>
void get_exp_alpha_field_wrapper(const real_t* coord, int n, real_t* out) {
  const real_t xc = 0.5;
  const real_t yc = 0.5;
...
template <class real_t>
void get_hopf_field_wrapper(const real_t* coord, int n, real_t* out) {
  const real_t xc = 0.5;
  const real_t yc = 0.5;
  const real_t zc = 0.5;
...
template <class real_t>
void get_diffusion_kernel_atT(const real_t* coord, int n, real_t* out) {
  const real_t amp = 1e-2;
  const real_t xc = 0.5;
  const real_t yc = 0.5;
  const real_t zc = 0.5;
...
template <class real_t>
void get_diffusion_kernel_hopf(const real_t* coord, int n, real_t* out) {
  const real_t amp = 1e-2;
  const real_t xc = 0.6;
  const real_t yc = 0.6;
  const real_t zc = 0.6;
  double time_curr = 2.5;
  tbslas::diffusion_kernel(coord, n, out, TBSLAS_DIFF_COEFF, time_curr, amp, xc,
                           yc, zc);
}

template <class real_t>
void get_gaussian_kernel_wrapper(const real_t* coord, int n, real_t* out) {
  const real_t xc = 0.5;
  const real_t yc = 0.5;
  const real_t zc = 0.55;
  tbslas::gaussian_kernel(coord, n, out, xc, yc, zc);
...
template <class real_t>
void get_diffusion_kernel_atT_hom(const real_t* coord, int n, real_t* out) {
  const real_t amp = 1e-2;
...
template <class real_t>
void fn_input_t2(const real_t* coord, int n, real_t* out) {
  tbslas::gaussian_kernel_diffusion_input(coord, n, out, TBSLAS_ALPHA);
}

template <class real_t>
void fn_poten_t2(const real_t* coord, int n, real_t* out) {
  tbslas::gaussian_kernel(coord, n, out);
}
...
void get_taylor_green_field_tv_ns_wrapper(const real_t* points_pos,
  ...
  for (int i = 0; i < num_points; i++) {
    ...
    /* z = p[2]-shiftz; */
    points_values[i * 3 + 0] =
        A * cos(a * x) * sin(b * y) * sin(c * z) * t_factor;
    points_values[i * 3 + 1] =
        B * sin(a * x) * cos(b * y) * sin(c * z) * t_factor;
    points_values[i * 3 + 2] =
        C * sin(a * x) * sin(b * y) * cos(c * z) * t_factor;
  }
}
...
void get_two_vortex_tubes_vorticity_field_ns(const real_t* points_pos,
  ...
  for (int i = 0; i < num_points; i++) {
    ...
    // FIRST TUBE
    {
      ...
      yc = 0;
      alpha = 2 * PI / 3.0;
      r2 = (x - (xc + A * cos(alpha) * (1 + cos(z)))) *
               (x - (xc + A * cos(alpha) * (1 + cos(z)))) +
           (y - (yc + A * sin(alpha) * (1 + cos(z)))) *
               (y - (yc + A * sin(alpha) * (1 + cos(z))));
      R = sqrt(r2) / r_cutoff;
      // omega = 0;
      if (R < 1) {
...
void get_two_vortex_tubes_curl_vorticity_field_ns(const real_t* points_pos,
  ...
  for (int i = 0; i < num_points; i++) {
    ...
    // FIRST TUBE
    {
      ...
      real_t dRdz = (A * cos(alpha) * sin(z) * xshifted +
                     A * sin(alpha) * sin(z) * yshifted) /
                    (r * r_cutoff);

      real_t dwdR = -omega0 * exp(-K * exp(1 / (R - 1)) / R) *
                    exp(1 / (R - 1)) *
                    (K / (R * R) + K / (R * (R - 1) * (R - 1)));
      ...
      if (R < 1) {
        ...
        points_values[i * 3 + 1] =
            scale_lentgh * (-A * cos(alpha) * tmp - dwdx);
        points_values[i * 3 + 2] =
            scale_lentgh *
            (-A * sin(alpha) * sin(z) * dwdx + A * cos(alpha) * sin(z) * dwdy);
      }
    }
    ...
    // SECOND TUBE
    {
      ...
      if (R < 1) {
        ...
        real_t tmp = cos(z) * omega + sin(z) * dwdz;
        points_values[i * 3 + 0] +=
            scale_lentgh * (dwdy + A * sin(alpha) * tmp);
        points_values[i * 3 + 1] +=
            scale_lentgh * (-A * cos(alpha) * tmp - dwdx);
        points_values[i * 3 + 2] +=
            scale_lentgh *
...
template <class real_t>
void get_taylor_green_field_ns(const real_t* points_pos, int num_points,
                               real_t* points_values) {
  ...
  for (int i = 0; i < num_points; i++) {
    ...
    z = p[2];
    points_values[i * 3 + 0] =
        A * sin(theta + m) * sin(a * x) * cos(b * y) * cos(c * z);
    points_values[i * 3 + 1] =
        B * sin(theta + n) * cos(a * x) * sin(b * y) * cos(c * z);
    points_values[i * 3 + 2] =
        C * sin(theta + q) * cos(a * x) * cos(b * y) * sin(c * z);
  }
}
