...

  {
    ...
    for (int timestep = 1; timestep < sim_config->total_num_timestep + 1;
          ...
          &comm, true);
      {
        ...
        tbslas::NodeFieldFunctor<double, Tree_t> tc_func =
            tbslas::NodeFieldFunctor<double, Tree_t>(tc);
        tbslas::FieldExtrapFunctor<double, Tree_t> te_func =
            tbslas::FieldExtrapFunctor<double, Tree_t>(tp, tc);
        tbslas::SolveSemilagInSitu(tc_func, te_func, tcon, timestep,
                                   sim_config->dt, sim_config->num_rk_step);
        pvfmm::Profile::Toc();
      }
