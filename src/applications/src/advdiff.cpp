...
void fn_input_t1(const Real_t* coord, int n, Real_t* out) {
...
template <class _FMM_Tree_Type, class _FMM_Mat_Type>
void RunFMM(_FMM_Tree_Type* tree, _FMM_Mat_Type* fmm_mat,
...
void RunAdvectDiff(int test_case, size_t N, size_t M, bool unif, int mult_order,
                   int cheb_deg, int depth, bool adap, Real_t tol,
                   MPI_Comm comm, int merge) {
  ...
  if (unif)
    ...
    pt_coord = tbslas::point_distrib<Real_t>(tbslas::RandElps, N,
                                             comm);  // RandElps, RandGaus
  ...
  tbslas::ConstructTree<FMM_Tree_t>(N, M, cheb_deg, depth, adap, tol, comm,
                                    fn_veloc_, 3, tvel_curr);
  if (sim_config->vtk_save_rate) {
    tvel_curr.Write2File(tbslas::GetVTKFileName(0, "velocity").c_str(),
                         cheb_deg);
  }
  tbslas::NodeFieldFunctor<double, FMM_Tree_t> tvel_func =
      tbslas::NodeFieldFunctor<double, FMM_Tree_t>(&tvel_curr);
  ...
  CheckChebOutput<FMM_Tree_t>(tree, fn_poten_, mykernel->ker_dim[1], in_al2,
  ...
  for (; timestep < 2 * NUM_TIME_STEPS + 1; timestep += 2) {
    ...
    tbslas::SolveSemilagInSitu(tvel_func, *tree, 1, TBSLAS_DT,
    ...
    // Write2File
    if (sim_config->vtk_save_rate) {
      tree->Write2File(tbslas::GetVTKFileName(timestep + 1,
                                              sim_config->vtk_filename_variable)
                           .c_str(),
                       sim_config->vtk_order);
    }
    tcurr += TBSLAS_DT;
  ...
  CheckChebOutput<FMM_Tree_t>(tree, fn_poten_, mykernel->ker_dim[1], al2, rl2,
...
int main(int argc, char** argv) {
  ...
  RunAdvectDiff<double>(test, sim_config->tree_num_point_sources,
                        ...
                        sim_config->tree_tolerance, comm, merge);
  pvfmm::Profile::Toc();
