...
template <class real_t>
void fn_input_t1(const real_t* coord, int num_points, real_t* out) {
  const real_t* p;
  for (int i = 0; i < num_points; i++) {
    p = &coord[i * COORD_DIM];
    out[i * 3 + 0] = p[1];
...
template <class real_t>
void fn_poten_t1(const real_t* coord, int num_points, real_t* out) {
  const real_t* p;
  for (int i = 0; i < num_points; i++) {
    p = &coord[i * COORD_DIM];
    out[i * 3 + 0] = 0;
...
template <class real_t>
void fn_input_t3(const real_t* coord, int num_points, real_t* out) {
  const real_t* p;
  for (int i = 0; i < num_points; i++) {
    p = &coord[i * COORD_DIM];
    out[i * 3 + 0] = 0;
...
template <class real_t>
void fn_poten_t3(const real_t* coord, int num_points, real_t* out) {
  const real_t* p;
  for (int i = 0; i < num_points; i++) {
    p = &coord[i * COORD_DIM];
    out[i * 3 + 0] = 0;
...
template <class real_t>
void fn_input_t4(const real_t* coord, int num_points, real_t* out) {
  const real_t* p;
  for (int i = 0; i < num_points; i++) {
    p = &coord[i * COORD_DIM];
    out[i * 3 + 0] = -p[1];
...
template <class real_t>
void fn_poten_t4(const real_t* coord, int num_points, real_t* out) {
  const real_t* p;
  for (int i = 0; i < num_points; i++) {
    p = &coord[i * COORD_DIM];
    out[i * 3 + 0] = 0;
...
void RunNS(int test, size_t N, size_t M, bool unif, int mult_order,
  ...
  //   (tbslas::GetModfiedStokesKernelName<real_t>(TBSLAS_MOD_STOKES_ALPHA,
  //   TBSLAS_MOD_STOKES_DIFF_COEFF), 3, std::pair<int,int>(3,3));
  ...
  tbslas::ConstructTree<FMM_Tree_t>(
      ...
      sim_config->tree_tolerance, comm, fn_input_, 3, *ti);

  tbslas::ConstructTree<FMM_Tree_t>(
      sim_config->tree_num_point_sources,
      sim_config->tree_num_points_per_octanct, sim_config->tree_chebyshev_order,
      sim_config->tree_max_depth, sim_config->tree_adap,
      sim_config->tree_tolerance, comm, fn_input_, 3, *tf);

  tbslas::ConstructTree<FMM_Tree_t>(
      sim_config->tree_num_point_sources,
      sim_config->tree_num_points_per_octanct, sim_config->tree_chebyshev_order,
      sim_config->tree_max_depth, sim_config->tree_adap,
      sim_config->tree_tolerance, comm, fn_poten_, 3, *tfa);

  // =========================================================================
  // COMPUTE INPUT ERROR
  // =========================================================================
  real_t in_al2, in_rl2, in_ali, in_rli;
  CheckChebOutput<FMM_Tree_t>(ti, fn_input_, 3, in_al2, in_rl2, in_ali, in_rli,
                              std::string("Input"));
  ...
  CheckChebOutput<FMM_Tree_t>(tf, fn_poten_, 3, al2, rl2, ali, rli,
                              std::string("Output"));
  ...
  if (!myrank) {
    ...
    // Rep::AddData("VAvgCFL", cfl_sum/(sim_config->total_num_timestep+1)); //
    // NUMBER OF TIMESTEPS + INITIAL TREE Rep::AddData("VMaxCFL", cfl_max);
...
int main(int argc, char** argv) {
  ...
  RunNS<double>(test, sim_config->tree_num_point_sources,
                sim_config->tree_num_points_per_octanct, unif, m,
                sim_config->tree_chebyshev_order, sim_config->tree_max_depth,
                sim_config->tree_adap, sim_config->tree_tolerance, merge, comm);
  pvfmm::Profile::Toc();
