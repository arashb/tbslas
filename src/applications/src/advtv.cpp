...
int main(int argc, char** argv) {
...

  {
    ...
    tbslas::FieldSetFunctor<double, Tree_t> vel_field_functor(vel_field_elems,
                                                              vel_field_times);

    if (sim_config->vtk_save_rate) {
      for (int i = 1; i < 4; i++) {
        vel_field_elems[i]->Write2File(
            tbslas::GetVTKFileName(i - 1, "vel").c_str(),
            sim_config->vtk_order);
      }
    }
    ...
    for (int timestep = 1; timestep < sim_config->total_num_timestep + 1;
          ...
          &comm, true);
      {
        ...
        tbslas::SolveSemilagInSitu(vel_field_functor, tcon, timestep,
