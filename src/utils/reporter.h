...
class Reporter {
 public:
  ...
  static void AddData(std::string header, _Real_Type value,
  ...
  static void Report() {
    ...
    for (int i = 0; i < header_vec.size(); ++i) {
      std::cout << std::right << std::setw(aligns_vec[i]) << header_vec[i];
    }
    std::cout << std::endl;
    // REPORT VALUES
    ...
    for (int i = 0; i < header_vec.size(); ++i) {
      switch (types_vec[i]) {
        case tbslas::REP_DOUBLE:
          std::cout << std::right << std::setw(aligns_vec[i])
                    << std::setprecision(2) << std::scientific << values_vec[i];
          break;
        case tbslas::REP_INT:
          std::cout << std::right
...
template <typename _Real_Type>
std::vector<std::string> Reporter<_Real_Type>::header_vec;

template <typename _Real_Type>
std::vector<_Real_Type> Reporter<_Real_Type>::values_vec;

template <typename _Real_Type>
std::vector<int> Reporter<_Real_Type>::aligns_vec;

template <typename _Real_Type>
std::vector<tbslas::Rep_Data_Type> Reporter<_Real_Type>::types_vec;
