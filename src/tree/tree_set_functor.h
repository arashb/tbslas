...
class FieldSetFunctor {
 public:
  explicit FieldSetFunctor(std::vector<Tree_t *> field_set_elems,
      ...
      : field_set_elems_(field_set_elems), field_set_times_(field_set_times) {
  ...
  void operator()(const Real_t *query_points_pos, int num_points,
                  Real_t query_time, Real_t *out) {
    ////////////////////////////////////////////////////////////////////////
    ...
    for (int i = 0; i < field_set_elems_.size(); i++) {
      std::vector<Real_t> *pPoints_val =
          new std::vector<Real_t>(num_points * data_dof_);
    ...
    for (int i = 0; i < field_set_points_val[0]->size(); i++) {
      Real_t grid_vals[4] = {
          (*field_set_points_val[0])[i], (*field_set_points_val[1])[i],
          (*field_set_points_val[2])[i], (*field_set_points_val[3])[i]};
