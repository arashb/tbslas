 ...
 */
template <class T>
void modified_stokes_vel(T *r_src, int src_cnt, T *v_src_, int dof, T *r_trg,
                         int trg_cnt, T *v_trg,
                         pvfmm::mem::MemoryManager *mem_mgr) {
  /* #ifndef __MIC__ */
  /*     pvfmm::Profile::Add_FLOP((long long)trg_cnt*(long
   * long)src_cnt*(28*dof)); */
  /* #endif */
    ...
    for (int i = 0; i < dof; i++) {
      ...
      for (int s = 0; s < src_cnt; s++) {
        ...
        if (R2 != 0) {
          ...
          T fr2 = COEEF *
                  (2 * invR3 * (1 - explambda) -
                   2 * lambda * invR2 * explambda - lambda2 * invR * explambda);
          T G = -fr2 - invR * fr;
          ...
          T v_src[3] = {v_src_[(s * dof + i) * 3],
                        ...
                        v_src_[(s * dof + i) * 3 + 2]};
          T inner_prod =
              (v_src[0] * dR[0] + v_src[1] * dR[1] + v_src[2] * dR[2]);
...
const pvfmm::Kernel<double> modified_stokes_vel_d =
    pvfmm::BuildKernel<double, modified_stokes_vel>(
        GetModfiedStokesKernelName(TBSLAS_MOD_STOKES_ALPHA,
                                   TBSLAS_MOD_STOKES_DIFF_COEFF),
        3, std::pair<int, int>(3, 3)); /* TODO: change the dimension */
...
template <>
const pvfmm::Kernel<double> &ModifiedStokesKernel<double>::velocity() {
